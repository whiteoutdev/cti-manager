...
function isEslintConfigAPluginModule(specifier) {
    return lodash.startsWith(specifier, 'plugin:');
}

function resolvePresetPackage(preset, rootDir) {
    if (isEslintConfigAPluginModule(preset)) {
        preset = preset.substring(7, preset.indexOf('/'));
        return `eslint-plugin-${preset}`;
    }
    // inspired from
    // https://github.com/eslint/eslint/blob/5b4a94e26d0ef247fe222dacab5749805d9780dd/lib/config/config-file.js#L347
